...
# 定义路径
fenzhiceshi_path = Path("trainsets/duozhang")  # 包含所有文件夹的路径
...
for folder in folders:
    ...
    if len(lines) >= 21:
        lines[20] = f'      , "dataroot_H": "trainsets/duozhang/{folder_name}"\n'
    else:
    ...
    # 将 185100_G.pth 剪切到新建文件夹中
    pth_file = models_path / "185100_G.pth"
    if pth_file.exists():
        shutil.move(str(pth_file), str(new_folder / "185100_G.pth"))
        print(f"已将 185100_G.pth 剪切到 {new_folder}。")
    else:
