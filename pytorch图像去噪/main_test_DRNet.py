...
def main():
    ...
    # ----------------------------------------
    # Preparation  python main_train_DRNet.py
    # ----------------------------------------

    noise_level_img = 25
    # noise level for noisy image
    model_name = '1040_G'
    testset_name = 'NWPU-RESISC45_val'           # test set, 'bsd68' | 'set12'
    ...
    for idx, img in enumerate(L_paths):
        ...
        if need_degradation:  # degradation process
            ...
            # 常见 speckle 噪声强度设置为 0.1 左右，可以调节
            #speckle_std = 0.1
            #noise = np.random.normal(0, speckle_std, img_L.shape)
            #img_L += img_L * noise
##############斑点噪声
            np.random.seed(seed=0)  # for reproducibility,
